...
class Optimization:
    ...
    """
    def __init__(self, model, loss_fn, optimizer, args, setting, device, is_retrain=False):
        self.model = model
        ...
        self.out_features = self.args.in_features - len(self.ctrl_vars) - len(self.ind_vars) - self.num_time_f
    ...
    def compute_dilate_loss(self, y, yhat, dilate_all):
        ...
        if self.is_retrain:
            ...
            if dilate_all:
                ...
                y = y[:, 0, -1].view([1, retrain_length, 1])
                yhat = yhat[:, 0, -1].view([1, retrain_length, 1])
        else:
            ...
            if dilate_all:
                ...
                y = y[:, :, -1].view([y.shape[0], self.args.pred_len, 1])
                yhat = yhat[:, :, -1].view([yhat.shape[0], self.args.pred_len, 1])
        ...
        '''
        loss, loss_shape, loss_temporal = dilate_loss(y[:, :, -1].view([batch_size, y.shape[1], 1]),
                                                            yhat[:, :, -1].view([batch_size, y.shape[1], 1]), 
    ...
    def train_step(self, x, y):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_amp):
            ...
            # Computes loss
            if loss_type == 'mse':
                loss = self.loss_fn(y, yhat, dilate_all)  
                # y_temp = y[:, :, -1].view([self.args.batch_size, self.args.pred_len, 1]).detach().cpu().numpy()
                # yhat_temp = yhat[:, :, -1].view([self.args.batch_size, self.args.pred_len, 1]).detach().cpu().numpy()
                # self.train_ys.append(np.concatenate((y_temp, yhat_temp), axis=2))                 
            
            if loss_type == 'dilate':
                loss, loss_shape, loss_temporal = self.compute_dilate_loss(y, yhat)
            ...
            with torch.no_grad():
                ...
                for x_val, y_val in val_loader:
                    ...
                    for k in range(self.args.batch_size):   
                        batch_size, N_output = y_val[:, :, -1].view([self.args.batch_size, self.args.pred_len, 1]).shape[0:2]
                        target = y_val[:, :, -1].view([self.args.batch_size, self.args.pred_len, 1])
                        outputs = yhat[:, :, -1].view([self.args.batch_size, self.args.pred_len, 1])
    ...
    def retrain(self, retrain_args, model, retrain_name, retrain_data, retrain_targets, 
        ...
        best_loss = None
        checkpoint_path = './checkpoints/' + self.args.setting + '/'
        ...
        for epoch in range(1, epochs+1):
                    ...
                    if retrain_mode == 'single_pred':
                            ...
                            with torch.no_grad():
                                ...
                                if use_actual_actions:
                                    if self.args.out_features < self.args.in_features - self.num_time_f:
                                        pred = torch.cat((y_true[:, :, :len(self.ctrl_vars)+len(self.ind_vars)], pred), dim=2)
                                    else:
                                        pred[:, :, :len(self.ctrl_vars)+len(self.ind_vars)] = y_true[:, :, :len(self.ctrl_vars)+len(self.ind_vars)] 
                        ...
                        with torch.no_grad():
                            ...
                            for i in range(batch_size):
                                ...
                                if use_actual_actions:
                                    if self.args.out_features < self.args.in_features - self.num_time_f:
                                        pred = torch.cat((y_true[:, 0, :len(self.ctrl_vars)+len(self.ind_vars)].unsqueeze(0), pred), dim=2)
                                    else:
                                        pred[:, :, :len(self.ctrl_vars)+len(self.ind_vars)] = y_true[:, :, :len(self.ctrl_vars)+len(self.ind_vars)] 
                        ...
                        targets = targets[:, :, len(self.ctrl_vars)+len(self.ind_vars):self.args.in_features-self.num_time_f].to(self.device)
            ...
            val_loss, val_loss_dtw, val_loss_tdi = self.val_retrain(self.model, val_loader, use_actual_actions)
    ...
    def retrain_sepp(self, model, train_loader, val_loader, epochs=10, use_actual_actions=False, 
                     max_el=120, retrain_name='', sepp_gap = False):
        ...
        for epoch in range(1, epochs+1):
                ...
                while idx + el + 1 < y_length:
                    ...
                    with torch.no_grad(), torch.cuda.amp.autocast():
                        ...
                        for i in range(el):
                            ...
                            pred = torch.cat((pred, y[:, :, self.args.out_features:].squeeze(0)), dim=1)
                            # print('prev_pred shape: ', prev_pred.shape)
                            x = torch.cat([x, pred.unsqueeze(0)], dim=1).to(self.device)
                            x = x[:, 1:, :]
                            x_list.append(x)
                            y_list.append(y_temp[idx+i+1][:,:,:self.args.out_features])
                    if sepp_gap:
                        idx = idx + el
                    else:
                        idx = idx + 1
                            
                x = torch.stack(x_list).view([-1, seq_len, self.args.in_features]).to(self.device)
                y = torch.stack(y_list).view([-1, pred_len, self.args.out_features]).to(self.device)
                # self.retrain_dict['x'].append(np.array(x.detach().cpu()))
                # self.retrain_dict['targets'].append(np.array(y.detach().cpu()))
                retrain = TensorDataset(x, y)
                retrain_loader = DataLoader(retrain, batch_size=batch_size, shuffle=False, drop_last=True)
                batch_losses = []
                for x_batch, y_batch in retrain_loader:
                    x_batch = x_batch.view([batch_size, -1, self.args.in_features]).to(self.device)
                    y_batch = y_batch.to(self.device)
                    loss = self.train_step(x_batch, y_batch)
                    batch_losses.append(loss)
                    
                episode_loss = np.mean(batch_losses)
                episode_losses.append(episode_loss)
                # self.retrain_dict['episode_losses'].append(episode_losses)

                if el == 1 or el % 10 == 0:
                    print(f'Episode Length {el} | Loss: {episode_loss:0.7f}')
                
            training_loss = np.mean(episode_losses)
            self.train_losses.append(training_loss)
            ...
            print(f'### [Epoch {epoch}/{epochs}] | Val Loss: {val_loss:.7f}  '+\
                  f'| Best loss: {best_loss:.7f} | Retrain loss : {training_loss:.7f} ###')

        return training_loss, val_loss, self.model, best_loss
    
    # Validation of the retrain results
    def val_retrain(self, model, val_loader, use_actual_actions):
            ...
            for x_val, y_val in val_loader:
                ...
                for i in range(val_length):
                    ...
                    if use_actual_actions:
                        if self.args.out_features < self.args.in_features - self.num_time_f:
                            pred = torch.cat((y_true[:, 0, :len(self.ctrl_vars)+len(self.ind_vars)].unsqueeze(0), pred), dim=2)
                            pred_list.append(pred[:, :, len(self.ctrl_vars)+len(self.ind_vars):self.args.in_features-self.num_time_f])
                        else:
                            pred[:, :, :len(self.ctrl_vars)+len(self.ind_vars)] = y_true[:, :, :len(self.ctrl_vars)+len(self.ind_vars)] 
                ...
                if self.args.out_features < self.args.in_features - self.num_time_f:
                    y_val = y_val[:, :val_length, len(self.ctrl_vars)+len(self.ind_vars):self.args.in_features-self.num_time_f].to(self.device)
                else:
                ...
                for k in range(y_val.shape[0]):   
                    batch_size, N_output = y_val[:, :, -1].view([1, val_length, 1]).shape[0:2]
                    target = y_val[:, :, -1].view([1, val_length, 1])
                    outputs = y_pred[:, :, -1].view([1, val_length, 1])
                    target_k_cpu = target[k,:,0:1].view(-1).detach().cpu().numpy()
                    output_k_cpu = outputs[k,:,0:1].view(-1).detach().cpu().numpy()
    ...
    def test_simulation(self, model, test_loader, ctrl_sep=False, ctrl_vals=None):
        """The method runs the simulation environment for a saved model

        The method takes a saved model, and simulation test loader. It runs the
        simulation environment for the length of each batch in test_loader
        using the saved model checkpoint
        ...
        Returns:
            test_results_list (list): List of the test results for each point 
            batch_test_losses (list): List of the losses of simulation for each point
        ...
        with torch.no_grad():
            ...
            for x_test, y_test in tqdm(test_loader):
                ...
                test_loss = self.loss_fn(y_real.detach().cpu(), y_pred.detach().cpu()).item()
                
                # DTW and TDI
                loss_dtw, loss_tdi = 0, 0
                for k in range(y_test.shape[0]):   
                    batch_size, N_output = y_test[:, :test_length, -1].view([1, test_length, 1]).shape[0:2]
                    target = y_real[:, -1].unsqueeze(0).view([1, test_length, 1])
                    outputs = y_pred[:, -1].unsqueeze(0).view([1, test_length, 1])
                    target_k_cpu = target[k,:,0:1].view(-1).detach().cpu().numpy()
                    output_k_cpu = outputs[k,:,0:1].view(-1).detach().cpu().numpy()
                ...
                test_results_dict['y_real'] = np.array(y_real.detach().cpu())
                test_results_dict['y_pred'] = np.array(y_pred.detach().cpu())
