...
class RandomDelayWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.prev_action_idx = 0  # TODO : initialize this better
        self.done_signal_sent = False
        first_observation = super().reset(**kwargs)[0]
    ...
    def step(self, action):
        ...
        if self.t < self.act_delay_range.stop and self.skip_initial_actions:
            ...
            m, r, d, trunc, info = self.env.step(self.next_action)  # before receive_action (e.g. rtrl setting with 0 delays)
            kappa, beta = self.receive_action()
            self.cum_rew_actor += r
            self.done_signal_sent = d
            self.send_observation((m, self.cum_rew_actor, d, trunc, info, kappa, beta))

        # at the brain again
        m, cum_rew_actor_delayed, d, trunc, info = self.receive_observation()
        ...
        # m or returned obs: (m, tuple(itertools.islice(self.past_actions, 0, self.past_actions.maxlen - 1)), alpha, kappa, beta)
        return m, r, d, trunc, info
    ...
    def receive_action(self):
        ...
        """
        print(f'Prev Action: {self.past_actions[self.prev_action_idx + 1]}\n')
        ...
        # find the index of the first element in self.arrival_times_actions where t <= self.t (0)
        # finds the first action index that is due (or overdue) based on the current time self.t
        ...
        print(f'Next Action: {self.next_action}\n')
    ...
    def receive_observation(self):
        ...
        m, r, d, trunc, info, kappa, beta = self.past_observations[alpha]
        # slice the self.past_actions deque from the beginning (0) up to one element less than its maximum length (self.past_actions.maxlen - 1)
        # The purpose here is to create a tuple of past actions, excluding the most recent action.
        return (m, tuple(itertools.islice(self.past_actions, 0, self.past_actions.maxlen - 1)), alpha, kappa, beta), r, d, trunc, info
