    ...
    def train(self):
        ...
        if not self.rtac:
            ...
            for i in reversed(range(self.act_buf_size)):  # we don't care about the delay of the first observation in the trajectory, but we care about the last one
                ...
                # at that position is updated to be (i - 1). Otherwise, nstep_len remains unchanged
            ...
            # scatter_: (in-place) fills the tensor created by torch.zeros with values specified by the last argument (1.) at the indices provided by nstep_len.unsqueeze(1).long().
