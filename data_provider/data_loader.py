...
class Dataset_Custom(Dataset):
    def __init__(self, df_predict, root_path, args, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h'):
        # size [seq_len, label_len, pred_len]
        # info
    ...
    def __read_data__(self):
        ...
        # print(cols)
        num_train = int(len(df_raw) * (1-self.args.test_ratio-0.1))
        ...
        if self.scale:
            ...
            joblib.dump(self.scaler, SCALER_PATH + 'scaler.gz')
        else:
            data = df_data.values
        
        # NOTE: data is the scaled valus of the whole dataset
        ...
        if self.set_type == 2:
            df_test = df_raw[border1s[2]:border2s[2]]
            df_test = df_test.set_index(['date'])
            df_test.index = pd.to_datetime(df_test.index)
            if not df_test.index.is_monotonic:
                df_test = df_test.sort_index()
                
            df_test.to_pickle(RESULTS_PATH + 'df_test.pkl')
...
# Retraining Dataset
class Dataset_Retrain(Dataset):
    def __init__(self, new_data, new_targets, args, df_raw, df_stamp, size=None, 
        ...
        self.data_x = self.__read_data__(new_data)
        self.data_y = self.__read_data__(new_targets)
    ...
    def __read_data__(self, df):
        ...
        index_range = range(self.df_raw.index.get_loc(df.date.iloc[0]),
                            self.df_raw.index.get_loc(df.date.iloc[-1])+1)

        self.data_stamp = self.df_stamp[index_range]
        # with open('data_stamp.pkl', 'wb') as file:
            # pickle.dump(self.data_stamp, file)
