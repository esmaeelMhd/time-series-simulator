...
class Retrainer():
    ...
    def set_experiment(self):
        ...
        if self.retrain_args.retrain_method == 'sepp':
            retr_method = 'SEPP'
            max_el = f'{self.retrain_args.max_episode_length}MaxEL'
            sepp_gap = '_Gap' if self.retrain_args.sepp_gap else ''

            self.retrain_name = '{} N{} - {}Ep_{}{}_{}_{}{}LR{}'.format(retr_method, self.retrain_number,
                                                                        self.retrain_args.retrain_epochs, actual_actions,
                                                                        sepp_gap, max_el, loss_type, self.retrain_args.retrain_lr,
                                                                        ss_setting)
        else:
    ...
    def build_model(self):
        ...
        if self.args.model == 'LSTM':
            if not hasattr(self.args, 'lstm_type') or (hasattr(self.args, 'lstm_type') and self.args.lstm_type != 'EncDec'):
                model = LSTMModel(self.args, self.device).float()
            else:
    ...
    def load_model(self, checkpoint_path):
        ...
        if os.path.isfile(os.path.join(checkpoint_path, self.retrain_name + '.pth')):
            ...
            self.loaded_model.load_state_dict(
                torch.load(checkpoint_path + 'checkpoint.pth'))
    ...
    def start_retrain_sepp(self):
        ...
        if self.args.model == 'LSTM':
            ...
            df_train, df_val = train_test_split(
                self.df, test_size=self.retrain_args.val_ratio, shuffle=False)

            for i in range(self.args.seq_len, len(df_train)-self.args.pred_len):
                X_train.append(df_train[i-self.args.seq_len:i])
                y_train.append(df_train[i:i+self.args.pred_len])

            train_data = torch.Tensor(np.array(X_train)).to(self.device)
            train_targets = torch.Tensor(np.array(y_train)).to(self.device)
            train_dataset = TensorDataset(train_data, train_targets)
            train_loader = DataLoader(
                train_dataset, batch_size=1, shuffle=False, drop_last=True)

            # Start the retrain process
            training_loss, validation_loss, self.loaded_model, self.best_loss = self.opt.retrain_sepp(model=self.loaded_model,
                                                                                                      train_loader=train_loader,
                                                                                                      val_loader=self.val_loader,
                                                                                                      epochs=self.retrain_args.retrain_epochs,
                                                                                                      use_actual_actions=self.retrain_args.use_actual_actions,
                                                                                                      max_el=self.retrain_args.max_episode_length,
                                                                                                      retrain_name=self.retrain_name,
                                                                                                      sepp_gap=self.retrain_args.sepp_gap)
