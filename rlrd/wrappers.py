...
class RealTimeWrapper(gym.Wrapper):
    ...
    def step(self, action):
        observation, reward, done, trunc, info = super().step(self.previous_action)
        self.previous_action = action
        return (observation, action), reward, done, trunc, info
...
class PreviousActionWrapper(gym.Wrapper):
    ...
    def step(self, action):
        observation, reward, done, trunc, info = super().step(action)  # this line is different from RealTimeWrapper
        self.previous_action = action
        return (observation, action), reward, done, trunc, info
...
class StatsWrapper(gym.Wrapper):
    ...
    def step(self, action):
        m, r, d, trunc, info = super().step(action)
...
class TimeLimitResetWrapper(gym.Wrapper):
    ...
    def step(self, action):
        m, r, d, trunc, info = self.env.step(action)
...
class Float64ToFloat32(gym.ObservationWrapper):
    ...
    def step(self, action):
        s, r, d, trunc, info = super().step(action)
        return s, r, d, trunc, info
...
class FrameSkip(gym.Wrapper):
    ...
    def step(self, action):
        ...
        for i in range(self.frame_skip):
            ...
            if d:
                break
        return m, reward * self.reward_scale, d, trunc, info
...
class RandomDelayWrapper(gym.Wrapper):
    ...
    def receive_observation(self):
        ...
        m, r, d, trunc, info, action_delay = self.past_observations[observation_delay]
        return (m, tuple(self.past_actions), observation_delay, action_delay), r, d, trunc, info
