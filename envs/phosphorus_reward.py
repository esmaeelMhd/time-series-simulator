...
class PhosphorusReward:
    ...
    data: Any = None
    target: str = None
    target_vars: Union[List[str], str] = None
    q_column: str = None
    ...
    def calculate_reward(self, source='env', state=None, actions=None, targets=None):
        ...
        if source == 'actual':
            ...
            return self._calculate_env_data_rewards(state, actions, targets)

    def _initialize_values(self):
        self.target_idx = self.target_vars.index(self.target)
    ...
    def _calculate_plant_data_rewards(self):
        ...
        target_column = self.data.iloc[:, self.data.columns.get_loc(self.target)]
        # Calculate the absolute deviations from the ideal target level
        deviations = abs(abs(target_column) - self.ideal_target_level)
        self.max_deviation = deviations.max()
        # Normalize deviations to have a relative scale
        norm_deviation = deviations / self.max_deviation

        # Calculate the tax based on phosphorus levels and flow rates
        target_kg = target_column * self.q_tank * self.freq_min / (60 * 1000)
        self.taxes = self.green_tax * target_kg
        # Normalize the tax to be between 0 and 1
        norm_tax = (self.taxes - self.taxes.min()) / (self.taxes.max() - self.taxes.min())
        ...
        self.JSF_costs = self.JSF_price * JSF_L
        self.PAX_costs = self.PAX_price * PAX_L
        # Normalize costs for actions
        norm_JSF = (self.JSF_costs - self.JSF_costs.min()) / (self.JSF_costs.max() - self.JSF_costs.min())
        norm_PAX = (self.PAX_costs - self.PAX_costs.min()) / (self.PAX_costs.max() - self.PAX_costs.min())
    ...
    def _calculate_env_data_rewards(self, state, actions, targets):
        ...
        JSF_L = actions[-1][0] * self.freq_min / 60
        PAX_L = actions[-1][2] * self.freq_min / 60
        ...
        # TODO: check why it returns numpy array
        return reward[0]