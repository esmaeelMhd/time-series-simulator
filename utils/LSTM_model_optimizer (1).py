...
class Optimization:
    ...
    def __init__(self, initiator=None, model=None, loss_fn=None, optimizer=None, args=None,
        ...
        if self.is_policy:
            ...
            self.out_features = self.args.in_features - self.n_actions - self.n_ind - self.num_time_f
            ...
            for i, (data_item, target_item) in enumerate(tqdm(zip(retrain_data, retrain_targets))):
                ...
                for x, targets in retrain_loader:
                    ...
                    if retrain_mode == 'single_pred':
                        ...
                        targets = targets[:, :, self.n_actions+self.n_ind:self.args.in_features-self.num_time_f].to(self.device)
            ...
            for x_test, y_test in tqdm(test_loader):
                ...
                for i in range(test_length):
                    ...
                    if self.args.out_features < self.args.in_features - self.num_time_f:
                        ...
                        if self.is_policy:
                            pred[:, :, self.n_actions:-self.num_time_f] = y_true[:, :, self.n_actions:-self.num_time_f]
                        else:
                            pred[:, :, :self.n_actions+self.n_ind] = y_true[:, :, :self.n_actions+self.n_ind]         
                ...
                for k in range(y_test.shape[0]):   
                    batch_size, N_output = y_test[:, :test_length, self.target_idx].view([1, test_length, 1]).shape[0:2]
