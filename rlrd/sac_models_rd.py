...
class DelayedMlpModule(Module):
    def __init__(self, observation_space, action_space, is_Q_network, hidden_units: int = 256, obs_delay=True, act_delay=True, tbmdp=False):  # FIXME: action_space param is less
        """
        Args:
            observation_space:
                Tuple((
        ...
        self.obs_dim = observation_space[0].shape[-1]
        self.buf_size = len(observation_space[1])
        # print(f"DEBUG: MLP self.buf_size: {self.buf_size}")
        self.act_dim = observation_space[1][0].shape[-1]
        assert self.act_dim == action_space.shape[-1], f"action spaces mismatch: {self.act_dim} and {action_space.shape[0]}"
...
class DelayedLstmModule(Module):
    def __init__(self, observation_space, action_space, is_Q_network, hidden_units: int = 256, obs_delay=True, act_delay=True, tbmdp=False):  
        super().__init__()
        assert isinstance(observation_space, gym.spaces.Tuple)
        ...
        if self.tbmdp:
            ...
            input_dim += (self.act_dim + 2) * self.buf_size
        elif self.act_delay or self.obs_delay:
            input_dim += (self.act_dim + 1) * self.buf_size
        else:
            input_dim += self.act_dim * self.buf_size

        self.lstm = nn.LSTM(input_dim, hidden_units, batch_first=True)
        self.lin = Linear(hidden_units, hidden_units)
...
class LSTMActionValue(Sequential):
    def __init__(self, observation_space, action_space, hidden_units, act_delay=True, obs_delay=True, tbmdp=False):
        super().__init__(
            DelayedLstmModule(observation_space, action_space, is_Q_network=True, act_delay=act_delay, obs_delay=obs_delay, tbmdp=tbmdp),
            ReLU(),
            Linear(hidden_units, hidden_units), ReLU(),
            Linear(hidden_units, 2)  # reward and entropy predicted separately
        )
...
class LSTMPolicy(Sequential):
    def __init__(self, observation_space, action_space, hidden_units, act_delay=True, obs_delay=True, tbmdp=False):
        super().__init__(
            DelayedLstmModule(observation_space, action_space, is_Q_network=False, act_delay=act_delay, obs_delay=obs_delay, tbmdp=tbmdp),
            ReLU(),
            Linear(hidden_units, hidden_units), ReLU(),
            TanhNormalLayer(hidden_units, action_space.shape[0])
        )
...
class LSTM(ActorModule):
    def __init__(self, observation_space, action_space, hidden_units: int = 256, num_critics: int = 2, act_delay: bool = True, obs_delay: bool = True, tbmdp: bool = False):
        ...
        self.min_action = action_space.low
        self.max_action = action_space.high
        self.critics = ModuleList(LSTMActionValue(observation_space, action_space, hidden_units, act_delay=act_delay, obs_delay=obs_delay, tbmdp=tbmdp) for _ in range(num_critics))
        self.actor = LSTMPolicy(observation_space, action_space, hidden_units, act_delay=act_delay, obs_delay=obs_delay, tbmdp=tbmdp)
        self.critic_output_layers = [c[-1] for c in self.critics]
