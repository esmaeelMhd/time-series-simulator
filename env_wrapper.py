...
class RandomDelayWrapper(gym.Wrapper):
    """
    Random Delay Wrapper
    """
    def __init__(self, env, obs_delay_range=range(0, 90), act_delay_range=range(0, 10), initial_action=None, skip_initial_actions=False):
        super().__init__(env)
    ...
    def calc_obs_delay(self, obs):
        v_tank = 7130       # Biology tank volume: m3
        sensor_place = 3/4  # Sensor placement at the tank
        q_in = self.wrapped_env.q_ep[self.t] 
        if q_in == 0:
            q_in += 0.001
        obs_delay = (sensor_place * v_tank) / (q_in / 60) / self.wrapped_env.freq_min
    ...
    def reset(self, **kwargs):
        ...
        # self.obs_delay_range = range(0, self.wrapped_env.max_delay)
