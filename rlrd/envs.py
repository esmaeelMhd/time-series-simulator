...
class Env(gym.Wrapper):
    ...
    def __init__(self, env, store_env=False):
        super().__init__(env)
        self.transition = (self.reset(), 0., True, False, {})
    ...
    def step(self, action):
        ...
        self.transition = next_state, reward, done, trunc, info
...
class GymEnv(Env):
    def __init__(self, env=None, seed_val=0, id: str = "Pendulum-v0", real_time: bool = False, frame_skip: int = 0, obs_scale: float = 0., store_env: bool = False):
        if env == None:
            env = gym.make(id)
